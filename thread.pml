...
inline propagate_command_in_parallel(command)
{
    ...
    :: j < 3 ->
            if
            :: command == START -> waiit_for_START_mask(j)
            :: command == CONTINUE -> waiit_for_RESUME_mask(j)
            :: command == PAUSE -> waiit_for_PAUSE_mask(j)
            :: command == STOP -> waiit_for_HALT_mask(j)
            fi
            j++
    :: else ->
            j = 1;
