...
ltl_file = open('ltl.prop', 'r')
lines = ltl_file.readlines()
print(lines)
for line in lines:
	prop = line.split('=')
	ltl[prop[0]] = prop[1]
...
for mutant in mutants:
	...
	#res = subprocess.run(['gcc', '-DNOREDUCE', '-DNP', '-o', 'pan', 'pan.c'], capture_output=True)
	#to_print = res.stdout.decode('utf-8')
	...
	for prop in properties:
		...
		#try:
		#	res = subprocess.run(['./pan', '-a', '-l', '-f', '-n', '-m100000', '-w30', '-N', prop], capture_output=True, timeout=time_limit)
		...
		#	print('spin violated \t'+mutant+'#'+prop+'\n')
		#	killing_report[prop].append(mutant)
		#	if mutant in survivors:
		#		survivors.remove(mutant)
		#	diag += (prop+'\t\t'+ mutant +'\t\t violated\n')
		#else:
		#	print('spin satisfied \t'+mutant+'#'+prop+'\n')
		#	diag += (prop+'\t\t'+ mutant +'\t\t satisfied\n')
		
		res = subprocess.run(['provelines', '-check', '-exhaustive', '-spin', '-ltl', ltl[prop]], capture_output=True)
		
		to_print = res.stdout.decode('utf-8')
		
		if to_print.find('problem found') != -1:
			print('provelines violated \t'+mutant+'#'+prop+'\n')
			killing_report[prop].append(mutant)
			if mutant in survivors:
				survivors.remove(mutant)
			diag += (prop+'\t\t'+ mutant +'\t\t violated\n')
		else:
			print('provelines satisfied \t'+mutant+'#'+prop+'\n')
			diag += (prop+'\t\t'+ mutant +'\t\t satisfied\n')
